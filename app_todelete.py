...
# JSON file paths for all insurance categories - pointing to todelete directory
JSON_FILES = {
    "health": Path(__file__).parent / "todelete" / "Health _Plans.json",
    "pension": Path(__file__).parent / "todelete" / "Pension_Plans.json", 
    "protection": Path(__file__).parent / "todelete" / "Protection_Plans.json",
    "savings": Path(__file__).parent / "todelete" / "Savings_Plans.json",
    "ulip": Path(__file__).parent / "todelete" / "ULIP_Plans.json",
    "annuity": Path(__file__).parent / "todelete" / "Annuity_Plans.json"
}

CATALOG_PATH = Path(__file__).parent / "todelete" / "Bank_infos_complete.json"
...
if __name__ == "__main__":
    ...
    uvicorn.run("app_todelete:app", host="0.0.0.0", port=8001, reload=True)
